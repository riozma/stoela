-- Bug: join nutzte lager_leiter.anmeldung_leiter_id, diese Spalte existiert
-- nicht auf lager_leiter (nur id, lager_id, profile_id, rolle, status).
-- Dadurch schlug get_lager_tn_anmeldung_info() IMMER mit SQL-Fehler fehl
-- ("column does not exist"), egal ob der Lager-Status passte -> TN-
-- Anmeldung zeigte fälschlich "Anmeldung nicht verfügbar". Fix: Join über
-- lager_id + profile_id statt über die nicht existierende Spalte.
